...
val_dataset = create_dataset('reds', args.data_root, img_type='bin', training=False)
...
for imgbatch, imgpaths, blurred_img in tqdm_loader:
    ...
    for i, (gt1, gt2, out1, out2) in enumerate(zip(gt_batch1, gt_batch2, out_batch1, out_batch2)):
        ...
        if psnr_parallel > psnr_crossed:
            ...
            lpips_metric = (lpips_metric11 + lpips_metric22) / 2 if psnr_ctr[i] == 0 else (lpips_metric11 + lpips_metric22 + lpips_metric_ctr[i]) / 3
            ...
            lpips_metric = (lpips_metric12 + lpips_metric21) / 2 if psnr_ctr[i] == 0 else (lpips_metric12 + lpips_metric21 + lpips_metric_ctr[i]) / 3
    ...
    '''
df.to_csv('{}/results_REDS.csv'.format(args.output_path))
